...
st.markdown("""
<style>
    header, [data-testid="stHeader"], [data-testid="stToolbar"], [data-testid="manage-app-button"], #MainMenu, footer, .stAppDeployButton, .viewerBadge_container {display: none !important; visibility: hidden !important;}
    .block-container {padding-top: 1rem !important; padding-bottom: 1.5rem !important;}
    div[data-testid="stVerticalBlock"] > div:has(#sticky-header) {position: -webkit-sticky !important; position: sticky !important; top: 0px !important; background-color: rgba(14, 17, 23, 0.98) !important; z-index: 99999 !important; padding: 5px 10px 10px 10px !important; border-bottom: 2px solid #1E3A8A !important; box-shadow: 0px 5px 15px rgba(0,0,0,0.5);}
    ...
    .kpi-ef-real { grid-column: 1; grid-row: 1; } .kpi-ef-prod { grid-column: 2; grid-row: 1; } .kpi-costo { grid-column: 3; grid-row: 1 / span 2; } .kpi-top-ef { grid-column: 1; grid-row: 2; } .kpi-top-imp { grid-column: 2; grid-row: 2; }
    h3 { white-space: nowrap !important; } .sub-title { font-size: 14px; color: #aaa; margin-top: -5px; margin-bottom: 15px; display: block; }
    @media (max-width: 1024px) {
        ...
        div[data-testid="stVerticalBlock"] > div:has(#sticky-header) {position: -webkit-sticky !important; position: sticky !important; top: 0px !important; padding: 5px !important; border-bottom: 2px solid #1E3A8A !important; box-shadow: 0px 5px 15px rgba(0,0,0,0.5) !important; z-index: 99999 !important;}
        ...
        div[data-testid="stHorizontalBlock"]:has(#header-anchor) > div:nth-child(1), div[data-testid="stHorizontalBlock"]:has(#header-anchor) > div:nth-child(3) { display: none !important; }
        ...
        .stMultiSelect div[data-baseweb="select"] { font-size: 13px !important; padding: 0 !important; min-height: 36px !important; height: 36px !important; border-radius: 6px !important;}
...
def mostrar_login():
    ...
    with c2:
        ...
        st.markdown("<div style='text-align:center;'><h2 style='color:#1E3A8A;'>GESTIÓN INDUSTRIAL PRODUCTIVA OMBÚ S.A.</h2><p>Acceso Restringido</p></div>", unsafe_allow_html=True)
        with st.form("form_login"):
            u_in = st.text_input("Usuario Corporativo"); p_in = st.text_input("Contraseña", type="password")
...
def generar_accion_sugerida(detalle):
    ...
    if any(x in d for x in ['ROTURA', 'FALLA', 'CORTE', 'MANTENIMIENTO', 'MECANICO']): return "⚙️ Revisar Equipo"
    if any(x in d for x in ['FALTA', 'MATERIAL', 'ESPERA', 'PUENTE', 'GRUA', 'INSUMO']): return "📦 Apurar Logística"
    if any(x in d for x in ['REPROCESO', 'CALIDAD', 'PLANO', 'ERROR']): return "🔎 Ajustar Calidad"
    if any(x in d for x in ['LIMPIEZA', 'ORDEN', '5S']): return "🧹 Optimizar 5S"
    if any(x in d for x in ['REUNION', 'CHARLA', 'CAPACITACION', 'PERSONAL']): return "👥 Gestionar RRHH"
    if any(x in d for x in ['PREPARACION', 'SET UP', 'SETUP', 'CAMBIO']): return "⏱️ Reducir Set-Up"
    return "⚡ Investigar Causa"

@st.cache_data(ttl=300)
def cargar_datos():
    df_ef = pd.read_excel("https://docs.google.com/spreadsheets/d/1_kO7GtjnlGHYgMnY7pkJGdRuFRAJB8xp8pGj8I0jq5E/export?format=xlsx")
    df_im = pd.read_excel("https://docs.google.com/spreadsheets/d/1GwshvXAotIShBPcX69vlE8_juO6w5aAFBCaXEsgSdCY/export?format=xlsx")
    ...
    cols_num = ['HH_STD_TOTAL', 'HH_Disponibles', 'Cant._Prod._A1', 'HH_Productivas_C/GAP', 'Costo_Improd._$', 'LUCRO_CESANTE_$', 'IMPACTO_TOTAL_$']
    c_std_u = next((c for c in df_ef.columns if 'STD' in str(c).upper() and ('UNID' in str(c).upper() or '/ U' in str(c).upper())), None)
    if c_std_u and c_std_u not in cols_num: cols_num.append(c_std_u)
    for col in cols_num:
...
try: df_ef, df_im, orig_col_pl, orig_col_li, orig_col_pu = cargar_datos()
except Exception as e: st.error(f"Error crítico cargando datos: {e}"); st.stop()
...
with st.container():
    ...
    with f_li: s_li = st.multiselect("⚙️ Línea", sorted(list(li_ef | li_im)), placeholder="⚙️ Línea...")
    if s_li:
    ...
    with f_pu: s_pu = st.multiselect("🛠️ Puesto", sorted(list(pu_ef | pu_im)), placeholder="🛠️ Puesto...")
...
for c in df_ef_f.columns:
    if 'PROD' in str(c).upper() and 'GAP' in str(c).upper() and 'UNID' not in str(c).upper() and '/ U' not in str(c).upper():
        col_prod_tot = c; break
...
top3_m1_html, top3_imp_html = "<div style='font-size:14px; color:#aaa; text-align:center;'>S/D</div>", "<div style='font-size:14px; color:#aaa; text-align:center;'>S/D</div>"
...
if not df_im_f.empty:
    ...
    if c_pu_im_top:
        ...
        if not ag_imp_p.empty:
            top3_imp_html = "".join([f"<div style='display:flex; justify-content:space-between; margin-top:4px; font-size:13px;'><span style='white-space:nowrap; overflow:hidden; text-overflow:ellipsis; max-width:140px;' title='{p}'>{i}. {p}</span><strong style='color:#FFCDD2; font-size:14px;'>{v:.1f}</strong></div>" for i, (p, v) in enumerate(ag_imp_p.items(), 1)])
...
<div class="kpi-grid">
    ...
    <div class="kpi-costo" style="background: linear-gradient(135deg, #D32F2F, #E53935); border: 1px solid #B71C1C; border-radius: 8px; display: flex; flex-direction: column; justify-content: center; text-align:center; box-shadow: 2px 4px 15px rgba(211,47,47,0.4); padding: 15px 10px;">
        ...
        <div style="display: flex; justify-content: space-between; background: rgba(0,0,0,0.25); padding: 8px; border-radius: 6px; margin-bottom: 10px;">
            <div style="width: 48%;"><p style="color: #FFCDD2; margin: 0; font-size: 11px; font-weight: bold; text-transform: uppercase;">Costo Directo M.O.</p><h4 style="color: white; margin: 0; font-size: 16px;">${tot_costo_mo:,.0f}</h4></div>
            <div style="border-left: 1px solid rgba(255,255,255,0.3); width: 4%;"></div>
            <div style="width: 48%;"><p style="color: #FFCDD2; margin: 0; font-size: 11px; font-weight: bold; text-transform: uppercase;">Lucro Cesante</p><h4 style="color: white; margin: 0; font-size: 16px;">${tot_lucro:,.0f}</h4></div>
        </div>
    ...
    <div class="kpi-top-ef" style="background: #0D47A1; color: white; border-radius: 6px; box-shadow: 2px 4px 10px rgba(0,0,0,0.3); padding: 10px;">
        <h4 style="font-size:14px; color:#BBDEFB; text-align:center; border-bottom: 1px solid rgba(255,255,255,0.2); margin:0 0 5px 0; padding-bottom:5px;">🏆 TOP EF. REAL</h4>{top3_m1_html}
    </div>
    <div class="kpi-top-imp" style="background: #B71C1C; color: white; border-radius: 6px; box-shadow: 2px 4px 10px rgba(0,0,0,0.3); padding: 10px;">
        <h4 style="font-size:14px; color:#FFCDD2; text-align:center; border-bottom: 1px solid rgba(255,255,255,0.2); margin:0 0 5px 0; padding-bottom:5px;">⚠️ TOP HH IMP.</h4>{top3_imp_html}
    </div>
</div>
""", unsafe_allow_html=True)
...
inc_hist_res, inc_act_res = 0.0, 0.0
if len(fechas_previas_res) > 0:
    disp_prev_res = df_ef_h_res[df_ef_h_res['Fecha'].isin(fechas_previas_res)]['HH_Disponibles'].sum()
    imp_prev_res = df_im_h_res[df_im_h_res['FECHA'].isin(fechas_previas_res)]['HH_IMPRODUCTIVAS'].sum()
...
if tot_disp_todas_res > 0: inc_act_res = tot_hh_imp / tot_disp_todas_res

ahorro_usd_res, ahorro_hh = 0, 0
...
tot_gp_crv26 = ag8_res['HH_STD_TOTAL'].sum() - ag8_res[col_prod_tot].sum() if not ag8_res.empty else 0
tot_gp_crv26 = tot_gp_crv26 / 130.0
...
<div style="background: #1A1C23; padding: 25px; border-radius: 12px; border: 2px solid #333; margin-top: 15px; margin-bottom: 20px; box-shadow: 0px 10px 20px rgba(0,0,0,0.6);">
    ...
    <div style="display: flex; justify-content: space-around; flex-wrap: wrap; gap: 20px;">
        <div style="text-align:center; min-width: 300px; flex: 1; padding: 15px; background: rgba(0,0,0,0.3); border-radius: 10px; border-top: 4px solid {res_color_econ};">
            ...
            <div style="margin-top: 15px; padding-top: 10px; border-top: 1px dashed #555;"><p style="color: #bbb; font-size: 13px; margin: 0; line-height: 1.5;"><b>¿De dónde sale?</b><br>({inc_hist_res*100:.1f}% Incidencia Histórica - {inc_act_res*100:.1f}% Incidencia Actual)<br>× <b>{tot_disp_todas_res:,.0f}</b> HH Disponibles</p></div>
        </div>
        <div style="text-align:center; min-width: 300px; flex: 1; padding: 15px; background: rgba(0,0,0,0.3); border-radius: 10px; border-top: 4px solid {res_color_prod};">
            ...
            <div style="margin-top: 15px; padding-top: 10px; border-top: 1px dashed #555;"><p style="color: #bbb; font-size: 13px; margin: 0; line-height: 1.5;"><b>¿De dónde sale?</b><br>({ag8_res['HH_STD_TOTAL'].sum() if not ag8_res.empty else 0:,.0f} HH Estándar - {ag8_res[col_prod_tot].sum() if not ag8_res.empty else 0:,.0f} HH Productivas)<br>÷ <b>130 HH</b> (Estándar 1 Máquina CRV 26)</p></div>
        </div>
    </div>
</div>
""", unsafe_allow_html=True)
...
with col1:
    ...
    if not df_plot_1.empty:
        ...
        ax1.legend(loc='lower left', bbox_to_anchor=(0, 1.05), ncol=2, frameon=True); ax1_line.legend(loc='lower right', bbox_to_anchor=(1, 1.05), frameon=True)
...
with col2:
    ...
    if not df_plot_1.empty:
        ...
        ax2.legend(loc='lower left', bbox_to_anchor=(0, 1.05), ncol=2, frameon=True); ax2_line.legend(loc='lower right', bbox_to_anchor=(1, 1.05), frameon=True)
...
with col4:
    ...
    if not df_ef_f.empty:
        ag4 = df_ef_f.groupby('Fecha').agg({'Costo_Improd._$': 'sum', 'LUCRO_CESANTE_$': 'sum', 'IMPACTO_TOTAL_$': 'sum'}).reset_index()
        if not df_im_f.empty:
            ag_im = df_im_f.groupby('FECHA')['HH_IMPRODUCTIVAS'].sum().reset_index().rename(columns={'FECHA':'Fecha', 'HH_IMPRODUCTIVAS':'HH_Imp'})
        ...
        for i in range(len(x_idx)):
            mo, lc, tot, hh = ag4['Costo_Improd._$'].iloc[i], ag4['LUCRO_CESANTE_$'].iloc[i], ag4['IMPACTO_TOTAL_$'].iloc[i], ag4['HH_Imp'].iloc[i]
            if mo > 0: ax4.text(x_idx[i], mo/2, f"${mo/1e6:.1f}M" if mo>=1e6 else f"${mo/1e3:.0f}K", ha='center', va='center', color='white', fontweight='bold', fontsize=11, path_effects=efecto_n, zorder=3)
            if lc > 0: ax4.text(x_idx[i], mo + (lc/2), f"${lc/1e6:.1f}M" if lc>=1e6 else f"${lc/1e3:.0f}K", ha='center', va='center', color='black', fontweight='bold', fontsize=11, path_effects=efecto_b, zorder=3)
            if tot > 0: ax4.annotate(f"${tot:,.0f}", (x_idx[i], tot), textcoords="offset points", xytext=(0, 12), ha='center', color='black', fontweight='bold', fontsize=12, bbox=caja_o, zorder=10)
            if hh > 0: ax4.annotate(f"⏱️ {hh:.1f} HH Imp.", (x_idx[i], tot), textcoords="offset points", xytext=(0, 40), ha='center', color='firebrick', fontweight='bold', fontsize=11, bbox=dict(boxstyle="round,pad=0.3", fc="#FFEBEE", ec="firebrick", lw=1.5), zorder=10)
...
with col5:
    ...
    if not df_im_f.empty:
        ...
        for i, causa in enumerate(ag5['TIPO_PARADA'].head(3).tolist()):
...
with col6:
    ...
    if not df_ef_f.empty:
        ...
        # --- AJUSTE V19: EXPANDIR GRAFICO HACIA ARRIBA Y MOVER LEYENDA ABAJO ---
        ...
        if list_c:
            ...
            for i, c_nom in enumerate(list_c):
                ...
                base_st += vals
            # Leyenda horizontal en la parte inferior (afuera del grafico)
        ...
        for i, val in enumerate(df6['Inc_%']): 
            if df6['Suma_I'].iloc[i] > 0: ax6_line.annotate(f"{val:.1f}%", (x_idx[i], val), textcoords="offset points", xytext=(0, 20), color='red', ha='center', fontsize=16, fontweight='bold', path_effects=efecto_b, zorder=10)
...
with col7:
    ...
    if not s_pl and not s_li and not s_pu:
        ...
        ag8_linea = df_m8_plant.groupby('Linea').agg({'HH_STD_TOTAL':'sum', col_prod_tot:'sum', 'HH_Disponibles':'sum', 'Cant._Prod._A1':'sum'}).reset_index()
        ...
        if not ag8_linea.empty:
            ...
            for i, (idx, row) in enumerate(ag8_linea.iterrows()):
                ef_a, ef_b, c_val, dif = row['A_val']*100, row['B_val']*100, row['C_val'], row['Dif_pct']
                estado_c = "PERDIDAS" if c_val >= 0 else "GANADAS"
                full_txt = f"[A] HH STD / HH DISP: {ef_a:.1f}%\n[B] HH STD / HH PROD: {ef_b:.1f}%\nDIFERENCIA (C):\n➤ {abs(c_val):.1f} U. {estado_c} AL 100% EF. REAL\n➤ {abs(c_val*0.85):.1f} U. {estado_c} AL 85% EF. REAL"
                ha_align = 'left' if dif >= 0 else 'right'
                offset_x = 10 if dif >= 0 else -10
                ax8.annotate(full_txt, xy=(dif, i), xytext=(offset_x, 0), textcoords="offset points", va='center', ha=ha_align, color='gold', fontweight='bold', fontsize=10, path_effects=efecto_n, bbox=dict(boxstyle="round,pad=0.5", fc="black", ec="gold", lw=1.5, alpha=0.85), zorder=10)
            yticklabels = [f"{textwrap.fill(str(row['Linea']), 20)}\n(Cant: {int(row['Cant._Prod._A1'])} U)" for _, row in ag8_linea.iterrows()]
            ax8.set_yticks(np.arange(len(ag8_linea))); ax8.set_yticklabels(yticklabels, fontsize=12, fontweight='bold')
            ax8.xaxis.set_major_formatter(mtick.PercentFormatter())
            idx_max_cb = ag8_linea['C_val'].idxmax()
            ax8.set_title(f"⚠️ CUELLO DE BOTELLA: {ag8_linea.loc[idx_max_cb, 'Linea']} (C: {ag8_linea.loc[idx_max_cb, 'C_val']:.1f})", color="firebrick", fontweight="bold", fontsize=18, pad=20)
            max_abs = ag8_linea['Dif_pct'].abs().max()
            ax8.set_xlim(-(max(max_abs*1.5, 60)) - 20, max(max_abs*1.5, 60) + 20)
            agregar_sello_agua(fig8); st.pyplot(fig8, use_container_width=True)
        else: st.warning("⚠️ Sin datos de producción para comparar líneas.")
    else:
        ...
        else: ag8['HH_Std_U'] = np.where(ag8['Cant._Prod._A1'] > 0, ag8['HH_STD_TOTAL'] / ag8['Cant._Prod._A1'], 0)
        ...
        if not ag8.empty:
            ...
            agregar_sello_agua(fig8); st.pyplot(fig8, use_container_width=True)
        else: st.warning("⚠️ Sin datos de producción.")
...
with col8:
    ...
    st.markdown(f"<div class='sub-title'><i>{'Competencia Sectorial vs Meta 85%' if tipo_ranking == 'Eficiencia Real' else 'Competencia Sectorial vs Meta 100%'}</i></div>", unsafe_allow_html=True)
    agrupar_por = 'Puesto_Trabajo' if (s_li or s_pu) else 'Linea'
    if agrupar_por in df_ef_f.columns:
        if agrupar_por == 'Linea':
            l_si = df_ef_f[df_ef_f['Es_Ultimo_Puesto'] == 'SI']['Linea'].unique()
            df_ranking = df_ef_f[(df_ef_f['Linea'].isin(l_si) & (df_ef_f['Es_Ultimo_Puesto'] == 'SI')) | (~df_ef_f['Linea'].isin(l_si))]
        else: df_ranking = df_ef_f
        ...
        if tipo_ranking == "Eficiencia Real":
            ag9 = ag9[ag9['HH_Disponibles'] > 0]; ag9['Ef'] = (ag9['HH_STD_TOTAL'] / ag9['HH_Disponibles']) * 100; meta_val = 85
        else:
            ag9 = ag9[ag9[col_prod_tot] > 0]; ag9['Ef'] = (ag9['HH_STD_TOTAL'] / ag9[col_prod_tot]) * 100; meta_val = 100
        ag9 = ag9.sort_values('Ef', ascending=True).reset_index(drop=True)
        if not ag9.empty:
            colors = ['firebrick' if val < (60 if meta_val==85 else 80) else 'gold' if val < (70 if meta_val==85 else 90) else 'lightgreen' if val < meta_val else 'darkgreen' for val in ag9['Ef']]
            fig9, ax9 = plt.subplots(figsize=(14, 10))
            
            fig9.subplots_adjust(top=0.95, bottom=0.15, left=0.25, right=0.95) 
            fig9.suptitle(t_enc, x=0.06, y=0.99, ha='left', fontsize=8, color='dimgray', fontweight='bold')
            ...
            ax9.set_yticks(np.arange(len(ag9))); ax9.set_yticklabels(yticklabels, fontsize=12, fontweight='bold')
            
            if meta_val == 85: leg_patches = [mpatches.Patch(color='firebrick', label='< 60%'), mpatches.Patch(color='gold', label='60% - 70%'), mpatches.Patch(color='lightgreen', label='70% - 85%'), mpatches.Patch(color='darkgreen', label='> 85%')]
            else: leg_patches = [mpatches.Patch(color='firebrick', label='< 80%'), mpatches.Patch(color='gold', label='80% - 90%'), mpatches.Patch(color='lightgreen', label='90% - 100%'), mpatches.Patch(color='darkgreen', label='> 100%')]
            ax9.legend(handles=leg_patches, loc='upper center', bbox_to_anchor=(0.5, -0.05), ncol=4, frameon=False, fontsize=11)
            
            agregar_sello_agua(fig9); st.pyplot(fig9, use_container_width=True)
        else: st.warning("⚠️ Sin datos para el ranking.")

st.markdown("---")

# =========================================================================
# 10. DETALLES DE IMPRODUCTIVIDAD
# =========================================================================
st.header("10. DETALLES DE IMPRODUCTIVIDAD")
st.markdown("<div class='sub-title'><i>Apertura de registros detallados con motor de sugerencia de acciones</i></div>", unsafe_allow_html=True)
if not df_im_f.empty and 'DETALLE' in df_im_f.columns:
    ...
    if not df_detalles.empty:
        ...
        ag_det = pd.concat([ag_det, pd.DataFrame({'Fecha':['---'], 'Operario':['---'], 'Puesto':['---'], 'Detalle Registrado':['✅ TOTAL'], 'Subtotal HH':[t_det], '%':[100.0], 'Acción Sugerida':['🎯']})], ignore_index=True)
