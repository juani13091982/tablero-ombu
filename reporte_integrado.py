...
# 3. MOTOR INTELIGENTE
# =========================================================================
def set_escala_y(ax, vmax, factor=1.8): # Factor ultra alto para evitar solapamientos
...
def cruce_robusto(sel, excel):
    """VERSIÓN CORREGIDA: Filtra estrictamente sin falsos positivos de números adentro de otros"""
    ...
    # Exigir separación por palabras y capturar desde 1 solo caracter (Ej: "1" no se mezcla con "10")
    p1, p2 = set(re.findall(r'[A-Z0-9]+', s1)), set(re.findall(r'[A-Z0-9]+', s2))
    excl = {'SECTOR', 'PUESTO', 'TRABAJO', 'LINEA', 'LÍNEA', 'PLANTA', 'AREA', 'MAQUINA'}
...
with st.container():
    ...
    with h_t: st.markdown("<h3 style='margin:0; padding:0;'>TABLERO INTEGRADO C.G.P.</h3>", unsafe_allow_html=True)
    with h_s:
    ...
    with col_filtros:
        ...
        s_pl = st.multiselect("Planta", list(df_ef['Planta'].dropna().unique()), label_visibility="collapsed", placeholder="🏭 Seleccionar Planta")
    ...
    top3_imp_html = "<div style='font-size:12px; color:#aaa; text-align:center;'>S/D</div>"
    if not df_im_f.empty:
...
with col1:
    ...
    if not df_plot_1.empty:
        ...
        fig1.subplots_adjust(top=0.80, bottom=0.22, left=0.08, right=0.92); fig1.suptitle(t_enc, x=0.08, y=0.98, ha='left', fontsize=8, color='dimgray', fontweight='bold')
        ...
        set_escala_y(ax1, ag1['HH_Disponibles'].max(), 1.8); ax1.bar_label(bs, padding=4, color='black', fontweight='bold', path_effects=efecto_b, fmt='%.0f', zorder=3); ax1.bar_label(bd, padding=4, color='black', fontweight='bold', path_effects=efecto_b, fmt='%.0f', zorder=3); dibujar_meses(ax1, len(x_idx))
        ...
        ax1_line.plot(x_idx, ag1['Ef_Real'], color='dimgray', marker='o', markersize=12, linewidth=4, path_effects=efecto_b, label='% Efic. Real', zorder=5); add_tendencia(ax1_line, x_idx, ag1['Ef_Real'])
        ...
        ax1_line.set_ylim(0, max(100, ag1['Ef_Real'].max()*1.4)); ax1_line.yaxis.set_major_formatter(mtick.PercentFormatter())
        for i, val in enumerate(ag1['Ef_Real']): ax1_line.annotate(f"{val:.1f}%", (x_idx[i], val + (ax1_line.get_ylim()[1]*0.04)), color='white', bbox=caja_g, ha='center', fontweight='bold', zorder=10)
        ax1.set_xticks(x_idx); ax1.set_xticklabels(ag1['Fecha'].dt.strftime('%b-%y'), fontsize=14, fontweight='bold'); ax1.legend(loc='lower left', bbox_to_anchor=(0, 1.05), ncol=2, frameon=True); ax1_line.legend(loc='lower right', bbox_to_anchor=(1, 1.05), frameon=True); agregar_sello_agua(fig1); st.pyplot(fig1)
...
with col2:
    ...
    if not df_plot_1.empty:
        ag2 = df_plot_1.groupby('Fecha').agg({'HH_STD_TOTAL': 'sum', 'HH_Productivas_C/GAP': 'sum'}).reset_index(); ag2['Ef_Prod'] = (ag2['HH_STD_TOTAL'] / ag2['HH_Productivas_C/GAP']).replace([np.inf, -np.inf], 0).fillna(0) * 100
        fig2, ax2 = plt.subplots(figsize=(14, 10)); ax2_line = ax2.twinx(); fig2.subplots_adjust(top=0.80, bottom=0.22, left=0.08, right=0.92); fig2.suptitle(t_enc, x=0.08, y=0.98, ha='left', fontsize=8, color='dimgray', fontweight='bold')
        x_idx = np.arange(len(ag2))
        bs = ax2.bar(x_idx - 0.17, ag2['HH_STD_TOTAL'], 0.35, color='midnightblue', edgecolor='white', label='HH STD TOTAL', zorder=2); bp = ax2.bar(x_idx + 0.17, ag2['HH_Productivas_C/GAP'], 0.35, color='darkgreen', edgecolor='white', label='HH PRODUCTIVAS', zorder=2)
        set_escala_y(ax2, max(ag2['HH_STD_TOTAL'].max(), ag2['HH_Productivas_C/GAP'].max()), 1.8); ax2.bar_label(bs, padding=4, color='black', fontweight='bold', path_effects=efecto_b, fmt='%.0f', zorder=3); ax2.bar_label(bp, padding=4, color='black', fontweight='bold', path_effects=efecto_b, fmt='%.0f', zorder=3); dibujar_meses(ax2, len(x_idx))
        ax2_line.plot(x_idx, ag2['Ef_Prod'], color='dimgray', marker='s', markersize=12, linewidth=4, path_effects=efecto_b, label='% Efic. Prod.', zorder=5); add_tendencia(ax2_line, x_idx, ag2['Ef_Prod'])
        ax2_line.axhline(100, color='darkgreen', linestyle='--', linewidth=3, zorder=1); ax2_line.text(x_idx[0], 101, 'META = 100%', color='white', bbox=caja_v, fontsize=14, fontweight='bold', zorder=10); ax2_line.set_ylim(0, max(110, ag2['Ef_Prod'].max()*1.4)); ax2_line.yaxis.set_major_formatter(mtick.PercentFormatter())
        for i, val in enumerate(ag2['Ef_Prod']): ax2_line.annotate(f"{val:.1f}%", (x_idx[i], val + (ax2_line.get_ylim()[1]*0.04)), color='white', bbox=caja_g, ha='center', fontweight='bold', zorder=10)
        ax2.set_xticks(x_idx); ax2.set_xticklabels(ag2['Fecha'].dt.strftime('%b-%y'), fontsize=14, fontweight='bold'); ax2.legend(loc='lower left', bbox_to_anchor=(0, 1.05), ncol=2, frameon=True); ax2_line.legend(loc='lower right', bbox_to_anchor=(1, 1.05), frameon=True); agregar_sello_agua(fig2); st.pyplot(fig2)
    else: st.warning("⚠️ Sin datos.")
...
with col3:
    st.header("3. GAP HH GLOBAL"); st.markdown("<div style='font-size:14px; color:#aaa; margin-top:-15px; margin-bottom:10px;'><i>Desvío entre Horas Disponibles y Declaradas Totales</i></div>", unsafe_allow_html=True)
    if not df_ef_f.empty:
        ...
        fig3, ax3 = plt.subplots(figsize=(14, 10)); fig3.subplots_adjust(top=0.80, bottom=0.22, left=0.08, right=0.92); fig3.suptitle(t_enc, x=0.08, y=0.98, ha='left', fontsize=8, color='dimgray', fontweight='bold')
        x_idx = np.arange(len(ag3))
        bp = ax3.bar(x_idx, ag3[c_prod], color='darkgreen', edgecolor='white', label='HH PRODUCTIVAS', zorder=2); bi = ax3.bar(x_idx, ag3['HH_Imp'], bottom=ag3[c_prod], color='firebrick', edgecolor='white', label='HH IMPRODUCTIVAS', zorder=2)
        ...
        for i in range(len(x_idx)):
            ...
            ax3.annotate(f"{int(hd)}", (i, hd + (ax3.get_ylim()[1]*0.04)), color='black', bbox=caja_b, ha='center', fontweight='bold', zorder=10)

        ax3.set_xticks(x_idx); ax3.set_xticklabels(ag3['Fecha'].dt.strftime('%b-%y'), fontsize=14, fontweight='bold'); ax3.legend(loc='lower left', bbox_to_anchor=(0, 1.05), ncol=3, frameon=True); agregar_sello_agua(fig3); st.pyplot(fig3)
    else: st.warning("⚠️ Sin datos para GAP.")

with col4:
    st.header("4. TENDENCIA COSTOS IMPRODUCTIVOS"); st.markdown("<div style='font-size:14px; color:#aaa; margin-top:-15px; margin-bottom:10px;'><i>Evolución de valorización económica de la ineficiencia</i></div>", unsafe_allow_html=True)
    if not df_ef_f.empty:
        ...
        fig4, ax4 = plt.subplots(figsize=(14, 10)); ax4_line = ax4.twinx(); fig4.subplots_adjust(top=0.80, bottom=0.22, left=0.08, right=0.92); fig4.suptitle(t_enc, x=0.08, y=0.98, ha='left', fontsize=8, color='dimgray', fontweight='bold')
        ...
        ax4_line.set_ylim(0, max(1000, ag4['Costo_Improd._$'].max() * 1.4)); ax4_line.set_yticklabels([f'${int(x/1000000)}M' for x in ax4_line.get_yticks()], fontweight='bold')
        ...
        ax4.set_xticks(x_idx); ax4.set_xticklabels(ag4['Fecha'].dt.strftime('%b-%y'), fontsize=14, fontweight='bold'); ax4.legend(loc='lower left', bbox_to_anchor=(0, 1.05), ncol=2, frameon=True); ax4_line.legend(loc='lower right', bbox_to_anchor=(1, 1.05), frameon=True); agregar_sello_agua(fig4); st.pyplot(fig4)
...
with col5:
    st.header("5. PARETO DE CAUSAS"); st.markdown("<div style='font-size:14px; color:#aaa; margin-top:-15px; margin-bottom:10px;'><i>Distribución de motivos de pérdida (80/20)</i></div>", unsafe_allow_html=True)
    if not df_im_f.empty:
        ...
        ax5_line.plot(x_idx, ag5['Pct_Acu'], color='red', marker='D', markersize=10, linewidth=4, path_effects=efecto_b, zorder=5); ax5_line.axhline(80, color='gray', linestyle='--', linewidth=2, zorder=1); ax5_line.set_ylim(0, 115); ax5_line.yaxis.set_major_formatter(mtick.PercentFormatter()) 
        ...
        for i, val in enumerate(ag5['Pct_Acu']): ax5_line.annotate(f"{val:.1f}%", (x_idx[i], val + 5), color='white', bbox=caja_g, ha='center', va='bottom', fontsize=11, rotation=45, zorder=10)
        agregar_sello_agua(fig5); st.pyplot(fig5)
    else: st.success("✅ ¡Felicitaciones! Cero horas improductivas en este periodo.")

with col6:
    st.header("6. EVOLUCIÓN INCIDENCIA %"); st.markdown("<div style='font-size:14px; color:#aaa; margin-top:-15px; margin-bottom:10px;'><i>Porcentaje histórico de HH Improductivas sobre Disponibles</i></div>", unsafe_allow_html=True)
    if not df_ef_f.empty:
        ...
        for i, val in enumerate(df6['Inc_%']): 
            if df6['Suma_I'].iloc[i] > 0: ax6_line.annotate(f"{val:.1f}%", (x_idx[i], val + (ax6_line.get_ylim()[1]*0.04)), color='red', ha='center', fontsize=16, fontweight='bold', path_effects=efecto_b, zorder=10)
...
if not df_im_f.empty and 'DETALLE' in df_im_f.columns:
    ...
    if not df_detalles.empty:
        ...
        ag_det = ag_det[['FECHA_STR', 'OPERARIO', c_pu_det, 'DETALLE', 'HH_IMPRODUCTIVAS', '%', 'Acción Sugerida']]; ag_det.columns = ['Fecha', 'Operario', 'Puesto', 'Detalle Registrado', 'Subtotal HH', '%', 'Acción Sugerida']
