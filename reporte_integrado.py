...
# =========================================================================
# 1. CONFIGURACIÓN DE PÁGINA Y ESCUDO VISUAL (LÍNEA 1)
# =========================================================================
...
# =========================================================================
# 3. MOTOR INTELIGENTE Y TENDENCIAS
# =========================================================================
...
def cruce_robusto(sel, excel):
    """Motor robusto de ayer a las 17hs: ignora palabras cortas y acentos, busca subsets"""
...
try:
    ...
    # Auto-Renombrado de Columnas Críticas
...
# =========================================================================
# 6. PROCESAMIENTO FINAL (FILTRADO DE BASES)
# =========================================================================
df_ef_f = df_ef.copy()
df_im_f = df_im.copy()
...
# Filtrar Improductivas de Forma Segura
...
# =========================================================================
# 7. FILA 1: MÉTRICAS 1 Y 2 (EFICIENCIAS) - CON LÓGICA DE SALIDA
# =========================================================================
col1, col2 = st.columns(2)

# Lógica del Embudo: Si selecciono Linea, priorizar Ultimo Puesto. Si no existe, avisar.
warn_linea = False
if s_pu:
    ...
    df_plot_1 = df_ef_f[df_ef_f['Es_Ultimo_Puesto'] == 'SI']
...
with col3:
    ...
    if not df_ef_f.empty:
        ...
        fig3.subplots_adjust(top=0.86, bottom=0.22, left=0.08, right=0.92); fig3.suptitle(t_enc, x=0.08, y=0.98, ha='left', fontsize=8, color='dimgray', fontweight='bold')
        ...
        for i in range(len(x_idx)):
            h_dis, h_dec = ag3['HH_Disponibles'].iloc[i], ag3['Total_Decl'].iloc[i]; gap = h_dis - h_dec
        ...
        ax3.legend(loc='lower left', bbox_to_anchor=(0, 1.02), ncol=3, frameon=True); st.pyplot(fig3)
...
with col4:
    ...
    if not df_ef_f.empty:
        ...
        tot_pesos, tot_hh_imp = ag4['Costo_Improd._$'].sum(), ag4['HH_Improductivas'].sum()
...
with col5:
    ...
    if not df_im_f.empty:
        ...
        # AQUÍ ESTABA EL ERROR: Faltaba este bloque para que no quede vacía si no hay paradas
        st.success("✅ ¡Felicitaciones! Cero horas improductivas en este periodo/filtro.")
...
with col6:
    ...
    if not df_ef_f.empty:
        ...
        for i, val in enumerate(df6['Inc_%']): 
            if df6['Suma_I'].iloc[i] > 0: # Solo pone etiqueta de % si hay improductividad real
...
# =========================================================================
# 10. FILA 4: ANÁLISIS PROFUNDO DE CAUSAS (DETALLES) - VUELVE A LA VIDA
# =========================================================================
