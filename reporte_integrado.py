...
<style>
    ...
    .main-header {
        ...
        padding: 20px; border-radius: 0px 0px 20px 20px;
    ...
    .oee-container {
        ...
        box-shadow: 0px 10px 25px rgba(0,0,0,0.05); margin-bottom: 25px;
    }
    .oee-value { font-size: 80px; font-weight: 900; margin: 0; line-height: 1; }
    .oee-label { font-size: 18px; color: #64748b; letter-spacing: 2px; font-weight: bold; }
    
    .pilar-grid { display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 15px; margin-top: 20px; }
    .pilar-card { padding: 15px; border-radius: 15px; background: #f8fafc; border: 1px solid #f1f5f9; }
    ...
    .kpi-card {
        ...
        border-left: 8px solid #1E3A8A; box-shadow: 0px 4px 10px rgba(0,0,0,0.03);
    }
</style>
...
# =========================================================================
# 3. CARGA DE DATOS (VERSION ULTRA-RESISTENTE)
# =========================================================================
def safe_sum(series):
    try:
        val = series.sum()
...
def load_data():
    ...
    d_ef = d_ef.rename(columns=m)
    d_ef = d_ef.loc[:, ~d_ef.columns.duplicated()]
    
    # Asegurar que existan aunque el Excel cambie
    for col in ['Fecha', 'Planta', 'HH_STD', 'HH_Disp', 'HH_Prod', 'Costo']:
        if col not in d_ef.columns: d_ef[col] = 0 if col != 'Fecha' and col != 'Planta' else 'S/D'
...
# 5. CÁLCULOS
# =========================================================================
h_std = safe_sum(dff['HH_STD'])
h_disp = safe_sum(dff['HH_Disp'])
h_prod = safe_sum(dff['HH_Prod'])
costo_tot = safe_sum(dff['Costo'])
...
<div class='oee-container'>
    ...
    <div style='background: #e2e8f0; height: 12px; border-radius: 10px; width: 60%; margin: 20px auto; overflow: hidden;'>
        <div style='background: {color_oee}; width: {min(max(oee_final,0), 100)}%; height: 100%;'></div>
    </div>
    <div class='pilar-grid'>
        <div class='pilar-card'>
            <div class='pilar-txt'>⏱️ Disponibilidad</div>
            <div class='pilar-val'>{min(disponibilidad, 100.0):.1f}%</div>
        </div>
        <div class='pilar-card'>
            <div class='pilar-txt'>🚀 Rendimiento</div>
            <div class='pilar-val'>{min(rendimiento, 100.0):.1f}%</div>
        </div>
        <div class='pilar-card'>
...
with c1: st.markdown(f"<div class='kpi-card'><h6>EFICIENCIA REAL</h6><h2 style='color:#1E3A8A;'>{(h_std/h_disp*100 if h_disp>0 else 0):.1f}%</h2></div>", unsafe_allow_html=True)
with c2: st.markdown(f"<div class='kpi-card' style='border-left-color:#22c55e;'><h6>TOTAL PRODUCIDO</h6><h2 style='color:#16a34a;'>{h_std:,.0f} <span style='font-size:15px;'>HH</span></h2></div>", unsafe_allow_html=True)
...
if not dff.empty:
    ...
    ax.plot(ag.index, ag['Ef'], marker='o', color='#1E3A8A', linewidth=3)
    ax.set_title("Evolución de Eficiencia Real")
    st.pyplot(fig)

st.success("✅ ¡Tablero Premium Blindado Online! OEEEEE!")
