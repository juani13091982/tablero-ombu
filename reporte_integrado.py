...
# =========================================================================
# 5. PANEL DE MANDO Y FILTROS LATERALES
# =========================================================================
st.markdown("---")
col_kpi, col_filtros = st.columns([3.5, 1], gap="large")

# --- LADO DERECHO: FILTROS APILADOS ---
with col_filtros:
    st.markdown("<h4 style='color:#4B8BBE; margin-bottom:10px;'>🎛️ FILTROS MAESTROS</h4>", unsafe_allow_html=True)
    ...
    s_pu = st.multiselect("🛠️ Puesto de Trabajo", list(df_tp['Puesto_Trabajo'].dropna().unique()))
...
# Top 3 Puestos (Ef Real)
top3_m1_html = "<div style='font-size:14px; color:#aaa; text-align:center;'>Sin datos suficientes</div>"
if not df_ef_f.empty:
    ...
    if not top3_val.empty:
        filas = [f"<div style='display:flex; justify-content:space-between; margin-top:5px; font-size:13px;'><span>{i}. {p[:18]}</span><strong style='color:#90CAF9;'>{v:.1f}%</strong></div>" for i, (p, v) in enumerate(top3_val.items(), 1)]
        top3_m1_html = "".join(filas)

# Top 3 Puestos (Improductivas)
top3_imp_html = "<div style='font-size:14px; color:#aaa; text-align:center;'>Sin datos suficientes</div>"
if not df_im_f.empty:
    ...
    if c_pu_im:
        ...
        if not ag_imp_p.empty:
            filas_imp = [f"<div style='display:flex; justify-content:space-between; margin-top:5px; font-size:13px;'><span>{i}. {p[:18]}</span><strong style='color:#FFCDD2;'>{v:.1f} HH</strong></div>" for i, (p, v) in enumerate(ag_imp_p.items(), 1)]
            top3_imp_html = "".join(filas_imp)

# --- LADO IZQUIERDO: GRILLA HTML DE CARTELES ---
with col_kpi:
    st.markdown(f"""
    <div style="display: grid; grid-template-columns: 1fr 1fr 1.3fr; gap: 15px; height:100%;">
        <div style="background: linear-gradient(135deg, #e0e0e0, #f5f5f5); border: 2px solid #aaa; border-left: 8px solid #1E3A8A; padding: 15px; border-radius: 8px; text-align:center; box-shadow: 2px 4px 10px rgba(0,0,0,0.3);">
            <h4 style="margin:0; color: #1E3A8A; font-size:15px;">EFICIENCIA REAL</h4>
            <h2 style="margin:5px 0 0 0; color: #111; font-size:36px;">{kpi_ef_real:.1f}%</h2>
        </div>
        <div style="background: linear-gradient(135deg, #2E7D32, #4CAF50); border: 2px solid #1B5E20; border-left: 8px solid #A5D6A7; padding: 15px; border-radius: 8px; text-align:center; box-shadow: 2px 4px 10px rgba(0,0,0,0.3);">
            <h4 style="margin:0; color: white; font-size:15px;">EFICIENCIA PROD.</h4>
            <h2 style="margin:5px 0 0 0; color: white; font-size:36px;">{kpi_ef_prod:.1f}%</h2>
        </div>
        <div style="background: linear-gradient(135deg, #D32F2F, #E53935); border: 2px solid #B71C1C; padding: 15px; border-radius: 12px; grid-row: span 2; display: flex; flex-direction: column; justify-content: center; text-align:center; box-shadow: 2px 4px 15px rgba(211,47,47,0.4);">
            <h4 style="margin:0; color: white; font-size:20px;">COSTO HH IMPROD.</h4>
            <p style="margin:0; color: #FFCDD2; font-size:13px;">(Oportunidad Perdida)</p>
            <h2 style="margin:15px 0; color: #FFEB3B; font-size:38px; text-shadow: 1px 1px 2px rgba(0,0,0,0.5);">${tot_costo:,.0f}</h2>
            <h4 style="margin:0; color: white; font-size:20px;">{tot_hh_imp:,.0f} <span style="font-size:14px; font-weight:normal;">HH PERDIDAS</span></h4>
        </div>
        <div style="background: #0D47A1; color: white; padding: 15px; border-radius: 8px; box-shadow: 2px 4px 10px rgba(0,0,0,0.3);">
            <h4 style="margin:0 0 10px 0; font-size:12px; color:#BBDEFB; text-align:center; border-bottom: 1px solid rgba(255,255,255,0.2); padding-bottom:5px;">🏆 TOP EF. REAL (PUESTOS)</h4>
            {top3_m1_html}
        </div>
        <div style="background: #B71C1C; color: white; padding: 15px; border-radius: 8px; box-shadow: 2px 4px 10px rgba(0,0,0,0.3);">
            <h4 style="margin:0 0 10px 0; font-size:12px; color:#FFCDD2; text-align:center; border-bottom: 1px solid rgba(255,255,255,0.2); padding-bottom:5px;">⚠️ TOP MAYOR HH IMP.</h4>
            {top3_imp_html}
        </div>
    </div>
    """, unsafe_allow_html=True)
...
with col1:
    ...
    if not df_plot_1.empty:
        ...
        ax1.bar_label(bs, padding=4, color='black', fontweight='bold', path_effects=efecto_b, fmt='%.0f', zorder=3); ax1.bar_label(bd, padding=4, color='black', fontweight='bold', path_effects=efecto_b, fmt='%.0f', zorder=3)
...
with col2:
    ...
    if not df_plot_1.empty:
        ...
        ax2.bar_label(bs, padding=4, color='black', fontweight='bold', path_effects=efecto_b, fmt='%.0f', zorder=3); ax2.bar_label(bp, padding=4, color='black', fontweight='bold', path_effects=efecto_b, fmt='%.0f', zorder=3)
...
# =========================================================================
# 8. FILA 2: MÉTRICAS 3 Y 4 (BRECHA Y COSTOS)
# =========================================================================
...
with col3:
    ...
    if not df_ef_f.empty:
        ...
        ax3.set_xticks(x_idx); ax3.set_xticklabels(ag3['Fecha'].dt.strftime('%b-%y'), fontsize=14, fontweight='bold'); ax3.legend(loc='lower left', bbox_to_anchor=(0, 1.02), ncol=3, frameon=True)
...
with col4:
    ...
    if not df_ef_f.empty:
        ...
        ax4.set_xticks(x_idx); ax4.set_xticklabels(ag4['Fecha'].dt.strftime('%b-%y'), fontsize=14, fontweight='bold'); ax4.legend(loc='lower left', bbox_to_anchor=(0, 1.02), ncol=2, frameon=True); ax4_line.legend(loc='lower right', bbox_to_anchor=(1, 1.02), frameon=True)
...
# =========================================================================
# 9. FILA 3: MÉTRICAS 5 Y 6 (PARETO E INCIDENCIA)
# =========================================================================
...
# =========================================================================
# 10. FILA 4: DETALLES DE IMPRODUCTIVIDAD (MESA DE TRABAJO)
# =========================================================================
