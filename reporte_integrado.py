...
with col2:
    ...
    if not df_plot_1.empty:
        ...
        ax2.set_xticks(x_idx); ax2.set_xticklabels(ag2['Fecha'].dt.strftime('%b-%y'), fontsize=14, fontweight='bold'); ax2.legend(loc='lower left', bbox_to_anchor=(0, 1.05), ncol=2, frameon=True); ax2_line.legend(loc='lower right', bbox_to_anchor=(1, 1.05), frameon=True); agregar_sello_agua(fig2); st.pyplot(fig2, use_container_width=True)
...
with col3:
    ...
    if not df_ef_f.empty:
        ...
        if not df_im_f.empty: ag3 = pd.merge(ag3, df_im_f.groupby('FECHA')['HH_IMPRODUCTIVAS'].sum().reset_index().rename(columns={'FECHA':'Fecha', 'HH_IMPRODUCTIVAS':'HH_Imp'}), on='Fecha', how='left').fillna(0)
        else: ag3['HH_Imp'] = 0
        ag3['Total_Decl'] = ag3[c_prod] + ag3['HH_Imp']
        fig3, ax3 = plt.subplots(figsize=(14, 10)); fig3.subplots_adjust(top=0.80, bottom=0.22, left=0.08, right=0.92); fig3.suptitle(t_enc, x=0.08, y=0.98, ha='left', fontsize=8, color='dimgray', fontweight='bold'); x_idx = np.arange(len(ag3))
        bp = ax3.bar(x_idx, ag3[c_prod], color='darkgreen', edgecolor='white', label='HH PRODUCTIVAS', zorder=2); bi = ax3.bar(x_idx, ag3['HH_Imp'], bottom=ag3[c_prod], color='firebrick', edgecolor='white', label='HH IMPRODUCTIVAS', zorder=2); ax3.bar_label(bp, label_type='center', color='white', fontweight='bold', fmt='%.0f', zorder=4); ax3.bar_label(bi, label_type='center', color='white', fontweight='bold', fmt='%.0f', zorder=4)
        ax3.plot(x_idx, ag3['HH_Disponibles'], color='black', marker='D', markersize=12, linewidth=4, path_effects=efecto_b, label='HH DISPONIBLES', zorder=5); set_escala_y(ax3, ag3['HH_Disponibles'].max(), 1.6); dibujar_meses(ax3, len(x_idx))
        for i in range(len(x_idx)):
            hd, ht = ag3['HH_Disponibles'].iloc[i], ag3['Total_Decl'].iloc[i]; gap = hd - ht; ax3.plot([i, i], [ht, hd], color='dimgray', linewidth=5, alpha=0.6, zorder=3); ax3.annotate(f"GAP:\n{int(gap)}", (i, ht + (gap/2)), color='firebrick', bbox=caja_b, ha='center', va='center', fontweight='bold', zorder=10); ax3.annotate(f"{int(hd)}", (i, hd + (ax3.get_ylim()[1]*0.05)), color='black', bbox=caja_b, ha='center', fontweight='bold', zorder=10)
        ax3.set_xticks(x_idx); ax3.set_xticklabels(ag3['Fecha'].dt.strftime('%b-%y'), fontsize=14, fontweight='bold'); ax3.legend(loc='lower left', bbox_to_anchor=(0, 1.05), ncol=3, frameon=True); agregar_sello_agua(fig3); st.pyplot(fig3, use_container_width=True)
